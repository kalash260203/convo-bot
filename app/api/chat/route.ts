...
export async function POST(request: NextRequest) {
  try {
    const { messages, apiKey, apiEndpoint } = await request.json()
...
function getDemoResponse(message: string, conversationLength: number = 0): string {
  const responses = [
...
async function getGeminiResponse(messages: ApiMessage[], apiKey?: string): Promise<string> {
